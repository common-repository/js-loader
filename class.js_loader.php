...
class Js_Loader {
	...
	public static function json_source() {
		...
		return JSLOADER__PLUGIN_DIR . '/libraries/jsdelivr.json';
	}
	...
	public function load_scripts(){
		...
		$files = self::rglob(plugin_dir_path( __FILE__ ) . '/libraries/custom/*', GLOB_NOSORT);
		...
		foreach ( $options as $key => $value ) {
			if( 1 == $value ){
				...
				foreach ($files as $key => $location) {
					rename($location, str_replace('.min.js', '.js', $location));
					rename($location, str_replace('.pkgd.js', '.js', $location));
				}
	...
	public static function enqueue_files($location){
